...
@section('title', 'Edit Kategori | ERP Tekstil')
...
<div class="max-w-3xl mx-auto pb-20">
    ...
    <div class="bg-white rounded-[3rem] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden">
        <div class="px-12 py-10 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
            <div>
                <h2 class="text-3xl font-black text-slate-900 tracking-tighter uppercase italic">Update Kategori</h2>
                <p class="text-sm text-slate-500 font-medium mt-1">Mengubah parameter klasifikasi #{{ $productCategory->code }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('product-categories.update', $productCategory) }}" class="p-12 space-y-10">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
                <div class="space-y-3">
                    ...
                    <input type="text" name="code" value="{{ old('code', $productCategory->code) }}" required
                ...
                <div class="md:col-span-2 space-y-3">
                    ...
                    <input type="text" name="name" value="{{ old('name', $productCategory->name) }}" required
            ...
            <div class="space-y-3">
                ...
                <textarea name="description" rows="4" 
                    class="w-full p-6 bg-slate-50 border border-slate-100 rounded-[2rem] text-sm font-medium focus:bg-white focus:ring-8 focus:ring-indigo-600/5 focus:border-indigo-600 outline-none transition-all shadow-inner">{{ old('description', $productCategory->description) }}</textarea>
            </div>

            <div class="flex items-center justify-end gap-8 pt-6">
                <a href="{{ route('product-categories.index') }}" class="text-[11px] font-black text-slate-400 hover:text-rose-600 uppercase tracking-widest transition-colors">Batalkan</a>
                <button type="submit"
