...
<div class="max-w-7xl mx-auto p-8 pb-20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        ...
        <div class="bg-slate-50 p-7 rounded-[2.5rem] border border-slate-200 shadow-sm text-slate-400">
            <p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">Bulan Ini</p>
            <h3 class="text-3xl font-black leading-none text-slate-600">{{ $prs->where('created_at', '>=', now()->startOfMonth())->count() }}</h3>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="p-8 border-b border-slate-50 bg-slate-50/20 flex justify-between items-center">
            <div class="relative w-full md:w-96">
                ...
                <input type="text" placeholder="Cari No. PR..." class="w-full pl-14 pr-6 py-4 bg-white border-none rounded-2xl text-sm font-bold shadow-sm ring-1 ring-slate-100 focus:ring-2 focus:ring-slate-900 transition-all">
            </div>
            <div class="flex gap-2">
                <button class="p-4 bg-white border border-slate-100 rounded-2xl text-slate-400 hover:text-slate-900 transition-all"><i data-lucide="filter" class="w-5 h-5"></i></button>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/50 text-slate-400 text-[10px] uppercase font-black tracking-[0.2em]">
                    <tr>
                        ...
                        <th class="px-10 py-6 text-right">Navigation</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50 text-sm font-medium">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-all group">
                        ...
                        <td class="px-10 py-6 text-right">
                            <a href="{{ route('purchase-requests.show', $pr->id) }}" class="inline-flex items-center justify-center w-12 h-12 bg-white border border-slate-100 text-slate-300 rounded-2xl hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all shadow-sm hover:shadow-xl hover:-translate-y-1">
