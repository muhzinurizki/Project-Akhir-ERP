    ...
    <div class="h-24 px-8 flex items-center shrink-0">
        <div class="flex items-center gap-4">
            <div class="relative group cursor-pointer">
                <div class="w-12 h-12 rounded-[1.25rem] bg-slate-900 flex items-center justify-center text-white font-black shadow-lg group-hover:rotate-6 transition-transform duration-300">
                    <span class="text-xl tracking-tighter">E</span>
                </div>
                <div class="absolute -top-1 -right-1 w-4 h-4 bg-emerald-500 border-[3px] border-white rounded-full"></div>
            </div>
        ...
        @foreach ($menus as $menu)
            @if (isset($menu['header']))
                <div class="pt-6 pb-2 px-4">
                    <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.25em]">{{ $menu['header'] }}</p>
                </div>
            @else
                @php
                ...
                <a href="{{ Route::has($menu['route']) ? route($menu['route']) : '#' }}"
                   class="group flex items-center justify-between px-3 py-2.5 rounded-2xl transition-all duration-300
                    ...
                    @if ($isActive)
                        <div class="w-1.5 h-5 bg-emerald-400 rounded-full shadow-[0_0_12px_rgba(52,211,153,0.6)] mr-1"></div>
                    @endif
                </a>
            @endif
        @endforeach
        ...
        <div class="p-4 rounded-[2rem] bg-slate-900 border border-slate-800 shadow-2xl">
            <div class="flex items-center gap-3">
                <div class="relative">
                    ...
                    <span class="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 border-2 border-slate-900 rounded-full shadow-sm"></span>
                </div>
            ...
            <div class="grid grid-cols-2 gap-2 mt-4">
                {{-- Tombol Pengaturan --}}
                <a href="#" class="flex items-center justify-center py-2 bg-slate-800 rounded-xl text-slate-400 hover:text-white hover:bg-slate-700 transition-all group/settings">
                    <i data-lucide="settings" class="w-3.5 h-3.5 group-hover/settings:rotate-45 transition-transform duration-500"></i>
                </a>

                {{-- Tombol Logout --}}
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-center py-2 bg-rose-500/10 rounded-xl text-rose-500 hover:bg-rose-500 hover:text-white transition-all group/logout">
                        <i data-lucide="log-out" class="w-3.5 h-3.5 group-hover/logout:translate-x-0.5 transition-transform"></i>
                    </button>
                </form>
            </div>
        </div>
