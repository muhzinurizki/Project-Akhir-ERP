...
<div class="space-y-8 pb-10">
        ...
        <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div class="space-y-4">
                ...
                <p class="text-slate-400 font-medium max-w-xl leading-relaxed">
                    Sistem ERP Tekstil memantau aliran produksi dan inventaris Anda secara cerdas. Semua data diperbarui secara otomatis.
                </p>
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        @php
        $stats = [
            ['label' => 'WO Aktif', 'value' => '24', 'target' => 'wo-active', 'icon' => 'activity', 'colorClass' => 'text-indigo-600', 'bgClass' => 'bg-indigo-50', 'hoverClass' => 'group-hover:bg-indigo-600'],
            ['label' => 'Stok Kritis', 'value' => '8', 'target' => 'stock-alert', 'icon' => 'alert-triangle', 'colorClass' => 'text-rose-600', 'bgClass' => 'bg-rose-50', 'hoverClass' => 'group-hover:bg-rose-600'],
            ['label' => 'PR Pending', 'value' => '12', 'target' => 'pr-pending', 'icon' => 'shopping-cart', 'colorClass' => 'text-amber-600', 'bgClass' => 'bg-amber-50', 'hoverClass' => 'group-hover:bg-amber-600'],
            ['label' => 'Total Users', 'value' => '42', 'target' => 'user-count', 'icon' => 'users', 'colorClass' => 'text-emerald-600', 'bgClass' => 'bg-emerald-50', 'hoverClass' => 'group-hover:bg-emerald-600'],
        ];
        @endphp
        ...
        <div class="group bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 relative overflow-hidden">
            ...
            <div class="flex items-baseline gap-2 mt-1">
                <p id="{{ $stat['target'] }}" class="text-4xl font-black text-slate-900 tracking-tighter transition-all duration-300">{{ $stat['value'] }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-[3rem] border border-slate-100 p-10 shadow-sm">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                <div>
                    ...
                    <p class="text-sm text-slate-400 font-medium">Monitoring output mesin secara real-time (Meter)</p>
                </div>
                <div class="flex bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
                    <button class="px-6 py-2 bg-white rounded-xl shadow-sm text-xs font-black text-slate-900">7 Hari</button>
                    <button class="px-6 py-2 text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors">30 Hari</button>
                </div>
            </div>
        ...
        <div class="bg-white rounded-[3rem] border border-slate-100 p-10 shadow-sm relative overflow-hidden group">
            ...
            <div class="space-y-8" id="activity-feed">
                ...
                <div class="relative pl-8 before:absolute before:left-0 before:top-1.5 before:w-3 before:h-3 before:bg-emerald-500 before:rounded-full before:ring-4 before:ring-emerald-50">
                ...
                <div class="relative pl-8 before:absolute before:left-0 before:top-1.5 before:w-3 before:h-3 before:bg-amber-500 before:rounded-full before:ring-4 before:ring-amber-50">
            ...
            <button class="w-full mt-10 py-4 rounded-[1.5rem] bg-slate-50 text-slate-600 text-[11px] font-black uppercase tracking-[0.2em] hover:bg-slate-900 hover:text-white transition-all shadow-sm">
                Buka Log Audit
            </button>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // 2. Production Chart Initialization
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Produksi',
                    data: [450, 520, 480, 610, 580, 690, 720],
                    borderColor: '#4f46e5',
                    borderWidth: 4,
                    fill: true,
        ...
        function pulseKPIs() {
            ...
            const randomID = stats[Math.floor(Math.random() * stats.length)];
            const el = document.getElementById(randomID);
            if(el) {
                el.classList.add('scale-110', 'text-indigo-600');
                setTimeout(() => el.classList.remove('scale-110', 'text-indigo-600'), 1000);
            }
        }

        setInterval(updateClock, 1000);
        setInterval(pulseKPIs, 4000);
        updateClock();
        lucide.createIcons();
    });
</script>
